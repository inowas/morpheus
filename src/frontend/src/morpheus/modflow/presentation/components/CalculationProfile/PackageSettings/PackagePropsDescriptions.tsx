...
export default {
  ...
  lak: {
    theta: <div>Time-weighting factor (Merritt and Konikow, 2000, p. 52).
      Explicit (THETA = 0.0), semi-implicit (0.0 &gt; THETA &gt; 1.0), or implicit
      (THETA = 1.0) solution for lake stages. SURFDEPTH, SSCNCR, NSSITR are read only if
      THETA is assigned a negative value (the negative value of THETA is
      then changed to a positive value internally by the code).
    </div>,
    nssitr: <div>Maximum number of iterations for Newtons method of solution for
      equilibrium lake stages in each MODFLOW iteration for steady-state
      aquifer head solution. If NSSITR = 0, a value of 100 will be used instead.</div>,
    sscncr: <div>Convergence criterion for equilibrium lake stage solution by Newtons
      method. Only read if ISS is not zero or if THETA is specified as a
      negative value. See notes above for nssitr.</div>,
    surfdepth: <div>The height of small topological variations (undulations) in lake-bottom
      elevations that can affect groundwater discharge to lakes. Values of SURFDEPTH ranging
      from 0.01 to 0.5 have been used successfully in test simulations.
      SURFDEP is read only if THETA is specified as a negative value.</div>,
    ipakcb: <div>TODO!!</div>,
