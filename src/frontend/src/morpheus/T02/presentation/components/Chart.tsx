...
const Chart: React.FC<IProps> = ({data, title, basinLength, basinWidth, chartHeight}) => {
  useEffect(() => {

    // Access Plotly from the global scope (window)
    ...
    Plotly.newPlot('plotlyContainer', plotData, config, layout);

    return () => {
      Plotly.purge('plotlyContainer');
    };
  }, [data, title, basinLength, basinWidth, chartHeight]);
