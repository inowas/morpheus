...
class EnableModelBoundaryPayload(TypedDict):
    project_id: str
    model_id: str
    boundary_id: str
...
class EnableModelBoundaryCommand(ProjectCommandBase):
    ...
    @classmethod
    def from_payload(cls, user_id: UserId, payload: EnableModelBoundaryPayload):
        return cls(
            user_id=user_id,
...
class EnableModelBoundaryCommandHandler(CommandHandlerBase):
    @staticmethod
    def handle(command: EnableModelBoundaryCommand):
        ...
        event = ModelBoundaryEnabledEvent.from_props(project_id=project_id, model_id=command.model_id, boundary_id=command.boundary_id, occurred_at=DateTime.now())
