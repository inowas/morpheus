...
class DisableModelBoundaryPayload(TypedDict):
    project_id: str
    model_id: str
    boundary_id: str
...
class DisableModelBoundaryCommand(ProjectCommandBase):
    model_id: ModelId
    boundary_id: BoundaryId

    @classmethod
    def from_payload(cls, user_id: UserId, payload: DisableModelBoundaryPayload):
        return cls(
            user_id=user_id,
...
class DisableModelBoundaryCommandHandler(CommandHandlerBase):
    @staticmethod
    def handle(command: DisableModelBoundaryCommand):
        ...
        event = ModelBoundaryDisabledEvent.from_props(project_id=project_id, model_id=command.model_id, boundary_id=command.boundary_id, occurred_at=DateTime.now())
