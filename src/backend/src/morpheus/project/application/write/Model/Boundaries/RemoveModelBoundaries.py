...
class RemoveModelBoundariesCommandHandler(CommandHandlerBase):
    ...
    def handle(command: RemoveModelBoundariesCommand):
        ...
        event = ModelBoundariesRemovedEvent.from_boundary_ids(project_id=project_id, model_id=command.model_id, boundary_ids=boundary_ids, occurred_at=DateTime.now())
