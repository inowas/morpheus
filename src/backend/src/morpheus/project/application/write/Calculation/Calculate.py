...
class CalculateCommandPayload(TypedDict):
    project_id: str
    model_id: str
...
class CalculateCommandHandler(CommandHandlerBase):
    @staticmethod
    def handle(command: CalculateCommand):
        ...
        event = CalculationQueuedEvent.from_calculation_id(project_id=project_id, calculation_id=calculation.calculation_id, occurred_at=DateTime.now())
