...
from ...infrastructure.persistence.ProjectSummaryRepository import ProjectSummaryRepository, project_summary_repository
...
from ...types.Project import Metadata, ProjectId, ProjectSummary
...
class ProjectReader:
    ...
    def get_project_summaries_with_user_privileges_for_identity(self, identity: Identity) -> list[tuple[ProjectSummary, list[Privilege]]]:
        ...
        return [
            (project_summary, PermissionService.get_privileges_for_identity_by_role_assignment_and_summary(identity, role_assignments, project_summary))
