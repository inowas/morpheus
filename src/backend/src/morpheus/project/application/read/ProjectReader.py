...
class ProjectReader:
    ...
    def get_project_summaries_with_role_for_identity(self, identity: Identity) -> list[tuple[ProjectSummary, Role | None]]:
        if identity.is_admin:
            return self._get_project_summaries_with_role_for_admin_user()

        return self._get_project_summaries_with_role_for_user(identity.user_id)

    def _get_project_summaries_with_role_for_admin_user(self) -> list[tuple[ProjectSummary, Role]]:
        return [(project_summary, Role.OWNER) for project_summary in self._project_summary_repository.find_all()]

    def _get_project_summaries_with_role_for_user(self, user_id: UserId) -> list[tuple[ProjectSummary, Role | None]]:
