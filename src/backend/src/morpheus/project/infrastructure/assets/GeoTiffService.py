...
from morpheus.project.types.Asset import GeoTiffAssetData, GeoTiffMetadata, NoDataValue
...
class GeoTiffService:
    ...
    def extract_asset_data(self, file: FilePath, band: int) -> GeoTiffAssetData:
        ...
        return GeoTiffAssetData(n_cols=metadata.n_cols, n_rows=metadata.n_rows, band=band, wgs_84_bounding_box=metadata.wgs_84_bounding_box, data=data)
    ...
    def _open_geo_tiff_in_wgs_84(self, file: FilePath) -> GeoTiff:
        try:
            ...
            raise InvalidGeoTiffException(f'Failed to read GeoTiff file: {e}') from e
    ...
    def _extract_metadata(self, geo_tiff: GeoTiff) -> GeoTiffMetadata:
        ...
        if min_value in [-9999.0, -32768.0, -3.4028235e38]:
            no_data_value = NoDataValue.from_float(min_value)

        return GeoTiffMetadata(n_cols=n_col, n_rows=n_row, n_bands=n_band, wgs_84_bounding_box=wgs_84_bounding_box, no_data_value=no_data_value)
