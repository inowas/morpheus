...
class PermissionsRepository(RepositoryBase):
    ...
    def update_ownership(self, project_id: ProjectId, user_id: UserId) -> None:
        ...
        self.collection.update_one(filter={'project_id': project_id.to_str()}, update={'$set': {'owner': user_id.to_str()}})
    ...
    def add_member(self, project_id: ProjectId, user_id: UserId, role: Role) -> None:
        ...
        self.collection.update_one(filter={'project_id': project_id.to_str()}, update={'$set': {'members': members.to_dict()}})
    ...
    def update_visibility(self, project_id: ProjectId, visibility: Visibility) -> None:
        ...
        self.collection.update_one(filter={'project_id': project_id.to_str()}, update={'$set': {'visibility': visibility.to_str()}})


permissions_repository = PermissionsRepository(
    collection=create_or_get_collection(get_database_client(app_settings.MONGO_PROJECT_DATABASE, create_if_not_exist=True), 'permissions')
)
