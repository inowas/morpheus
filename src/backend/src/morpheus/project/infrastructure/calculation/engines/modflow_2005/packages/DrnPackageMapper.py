...
def calculate_drn_boundary_stress_period_data(
    spatial_discretization: SpatialDiscretization, time_discretization: TimeDiscretization, layers: LayersCollection, drn_boundary: DrainBoundary
) -> DrnStressPeriodData:
    ...
    for stress_period_idx, _stress_period in enumerate(time_discretization.stress_periods):
        ...
        drn_boundary.affected_cells = drn_boundary.affected_cells.filter(lambda affected_cell: spatial_discretization.affected_cells.contains(affected_cell))
        ...
        if drn_boundary.number_of_observations() == 1:
            ...
            if not isinstance(mean_data, DrainDataItem):
                raise TypeError(f'Expected DrainDataItem but got {type(mean_data)}')
...
def calculate_stress_period_data(model: Model) -> DrnStressPeriodData | None:
    ...
    for drn_boundary in drn_boundaries:
        if not isinstance(drn_boundary, DrainBoundary):
            raise TypeError(f'Expected boundary of type {DrainBoundary.__name__} but got {type(drn_boundary)}')

        sp_data_boundary = calculate_drn_boundary_stress_period_data(
            spatial_discretization=model.spatial_discretization, time_discretization=model.time_discretization, layers=model.layers, drn_boundary=drn_boundary
        )
