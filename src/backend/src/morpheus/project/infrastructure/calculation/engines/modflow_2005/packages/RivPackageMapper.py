...
def calculate_riv_boundary_stress_period_data(
    spatial_discretization: SpatialDiscretization, time_discretization: TimeDiscretization, layers: LayersCollection, riv_boundary: RiverBoundary
) -> RivStressPeriodData:
    ...
    for stress_period_idx, _stress_period in enumerate(time_discretization.stress_periods):
        ...
        mean_data = riv_boundary.get_mean_data(start_date_time=start_date_time, end_date_time=end_date_time, interpolation=riv_boundary.interpolation)
        ...
        riv_boundary.affected_cells = riv_boundary.affected_cells.filter(lambda affected_cell: spatial_discretization.affected_cells.contains(affected_cell))
            ...
            for cell in riv_boundary.affected_cells:
                ...
                for layer_idx in layer_indices:
                    sp_data.set_value(
                        ...
                        row=cell.row,
                        column=cell.col,
                        values=[river_stage.to_float(), conductance.to_float(), riverbed_bottom.to_float()],
                        sum_up_values=False,
                    )
        ...
        if riv_boundary.number_of_observations() > 1:
            ...
            for observation in observations:
                ...
                mean_data = observation.get_data_item(start_date_time=start_date_time, end_date_time=end_date_time, interpolation=riv_boundary.interpolation)
                if not isinstance(mean_data, RiverDataItem):
                    raise TypeError(f'Expected GeneralHeadDataItem but got {type(mean_data)}')
...
def calculate_stress_period_data(model: Model) -> RivStressPeriodData | None:
    ...
    for riv_boundary in riv_boundaries:
        if not isinstance(riv_boundary, RiverBoundary):
            raise TypeError(f'Expected boundary of type {RiverBoundary.__name__} but got {type(riv_boundary)}')

        sp_data_boundary = calculate_riv_boundary_stress_period_data(
            spatial_discretization=model.spatial_discretization, time_discretization=model.time_discretization, layers=model.layers, riv_boundary=riv_boundary
        )
