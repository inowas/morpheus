...
class ReadCalculationResultsRequestHandler:
    ...
    def _get_result_data_object(self, data: np.array, grid: Grid):
        return {
            'n_cols': data.shape[1],
            'n_rows': data.shape[0],
            'outline': grid.get_wgs_outline_geometry().to_dict(),
            'rotation': grid.rotation.to_float(),
            'values': [[None if math.isnan(cell) else cell for cell in row] for row in data],
            'min_value': np.nanmin(data) if not np.isnan(np.nanmin(data)) else None,
            'max_value': np.nanmax(data) if not np.isnan(np.nanmax(data)) else None
        }
    ...
    def handle(self, project_id: ProjectId, calculation_id: CalculationId, result_type: str, time_idx: int, layer_idx: int = 0, incremental: bool = False):
        ...
        if result_type == 'flow_head':
            ...
            result = {
                ...
                'layer_idx': layer_idx,
                'time_idx': time_idx,
                'data': self._get_result_data_object(np.array(data), grid)
            }
