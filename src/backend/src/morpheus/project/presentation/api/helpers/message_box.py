...
def generate_response_for(command: CommandBase) -> ResourceCreated | None:
    if isinstance(command, CreateProjectCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}')

    if isinstance(command, AddCalculationProfileCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/calculation-profiles/{command.calculation_profile.id.to_str()}')

    if isinstance(command, AddModelObservationCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/observations/{command.observation_id.to_str()}')

    if isinstance(command, CreateModelCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model')

    if isinstance(command, CreateModelVersionCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/versions/{command.version_tag.to_str()}')

    if isinstance(command, CreateModelLayerCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/layers/{command.layer_id.to_str()}')

    if isinstance(command, CloneModelLayerCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/layers/{command.new_layer_id.to_str()}')

    if isinstance(command, CloneModelObservationCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/observations/{command.new_observation_id.to_str()}')

    if isinstance(command, AddModelBoundaryCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/boundaries/{command.boundary_id.to_str()}')

    if isinstance(command, CloneModelBoundaryCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/model/boundaries/{command.new_boundary_id.to_str()}')

    if isinstance(command, StartCalculationCommand):
        return ResourceCreated(f'projects/{command.project_id.to_str()}/calculations/{command.new_calculation_id.to_str()}')

    if isinstance(command, AddModelBoundaryObservationCommand):
        return ResourceCreated(
            f'projects/{command.project_id.to_str()}/model/boundaries/{command.boundary_id.to_str()}/observations/{command.observation_id.to_str()}',
        )
