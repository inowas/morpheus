...
class ProjectEventFactory:
    def create_event(self, event_name: EventName, entity_uuid: Uuid, occurred_at: DateTime, payload: dict):
        ...
        if event_name.to_str() == ProjectEventName.MODEL_LAYER_CONFINEMENT_UPDATED:
            return ModelLayerConfinementUpdatedEvent(entity_uuid=entity_uuid, occurred_at=occurred_at, payload=payload)
        ...
        if event_name.to_str() == ProjectEventName.MODEL_LAYER_METADATA_UPDATED:
            return ModelLayerMetadataUpdatedEvent(entity_uuid=entity_uuid, occurred_at=occurred_at, payload=payload)
        if event_name.to_str() == ProjectEventName.MODEL_LAYER_PROPERTY_UPDATED:
            return ModelLayerPropertyUpdatedEvent(entity_uuid=entity_uuid, occurred_at=occurred_at, payload=payload)
