...
class RechargeObservation(Observation):
    ...
    def get_data_item(self, start_date_time: StartDateTime, end_date_time: EndDateTime, interpolation: InterpolationType) -> RechargeDataItem | None:
        ...
        if interpolation == InterpolationType.forward_fill:
            df = pd.DataFrame({'time_series': time_series, 'recharge_rates': recharge_rates})
            df = df.set_index('time_series')
            df = df.reindex(date_range, method='ffill')

            target_date_time = pd.to_datetime(start_date_time.to_value())
            recharge_at_start_date_time = df.loc[target_date_time, 'recharge_rates']

            return RechargeDataItem(
                observation_id=self.observation_id,
                start_date_time=start_date_time,
                end_date_time=end_date_time,
                recharge_rate=RechargeRate.from_value(recharge_at_start_date_time)
            )
