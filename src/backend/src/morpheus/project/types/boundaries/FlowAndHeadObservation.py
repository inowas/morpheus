...
from .Observation import Observation, ObservationId, ObservationName
...
class FlowAndHeadRawDataItem:
    ...
    def default(cls, date_time: StartDateTime):
        return cls(date_time=date_time, flow=None, head=Head.from_float(0.0))
    ...
    def get_flow_data_item(self, date_time: StartDateTime) -> FlowDataItem | None:
        ...
        for item in self.data:
            if item.date_time == date_time:
                ...
                return FlowDataItem(observation_id=self.observation_id, date_time=date_time, flow=item.flow)
        return None
        ...
        for item in self.data:
            if item.date_time == date_time:
                if item.head is None:
                    return None
                return HeadDataItem(observation_id=self.observation_id, date_time=date_time, head=item.head)
        return None
