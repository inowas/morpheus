...
from morpheus.project.types.boundaries.ConstantHeadObservation import ConstantHeadObservationValue, ConstantHeadObservation
from morpheus.project.types.boundaries.DrainObservation import DrainRawDataItem, DrainObservation
from morpheus.project.types.boundaries.EvapotranspirationObservation import EvapotranspirationObservationValue, EvapotranspirationObservation
...
from morpheus.project.types.boundaries.LakeObservation import LakeObservationValue, LakeObservation, BedLeakance, InitialStage, StageRange
from morpheus.project.types.boundaries.Observation import Observation, ObservationName, ObservationId
from morpheus.project.types.boundaries.RechargeObservation import RechargeObservationValue, RechargeObservation
from morpheus.project.types.boundaries.RiverObservation import RiverObservationValue, RiverObservation
from morpheus.project.types.boundaries.WellObservation import WellObservationValue, WellObservation
...
class ObservationFactory:
    ...
    def new(boundary_type: BoundaryType, observation_name: ObservationName, observation_geometry: Point, observation_data: list, observation_id: ObservationId) -> Observation:
        if boundary_type == BoundaryType.constant_head:
            return ConstantHeadObservation.new(name=observation_name, geometry=observation_geometry, data=[ConstantHeadObservationValue.from_dict(item) for item in observation_data],
        ...
        if boundary_type == BoundaryType.lake:
            return LakeObservation.new(name=observation_name, geometry=observation_geometry, data=[LakeObservationValue.from_dict(item) for item in observation_data],
