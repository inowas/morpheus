...
class DrainObservation(Observation):
    raw_data: list[DrainRawDataItem]

    @classmethod
    def new(cls, name: ObservationName, geometry: Point, raw_data: list[DrainRawDataItem]):
        return cls(
    ...
    def from_dict(cls, obj):
        return cls(
            ...
            raw_data=[DrainRawDataItem.from_dict(d) for d in obj['raw_data']]
        )
    ...
    def get_data_item(self, start_date_time: StartDateTime, end_date_time: EndDateTime) -> DrainDataItem | None:
