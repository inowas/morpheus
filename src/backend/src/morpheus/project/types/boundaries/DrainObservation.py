...
class DrainRawDataItem:
    ...
    def default(cls, date_time: StartDateTime):
        return cls(date_time=date_time, stage=Stage.from_float(0.0), conductance=Conductance.from_float(0.0))

    @classmethod
    def from_dict(cls, obj):
        return cls(date_time=StartDateTime.from_value(obj['date_time']), stage=Stage.from_value(obj['stage']), conductance=Conductance.from_value(obj['conductance']))

    def to_dict(self):
        return {'date_time': self.date_time.to_value(), 'stage': self.stage.to_value(), 'conductance': self.conductance.to_value()}


@dataclasses.dataclass
class DrainObservation(Observation):
    ...
    def from_dict(cls, obj):
        return cls(
            ...
            data=[DrainRawDataItem.from_dict(d) for d in obj['data']],
        )
            ...
            for item in self.data:
                if item.date_time == start_date_time:
                    return DrainDataItem(
                        observation_id=self.observation_id, start_date_time=start_date_time, end_date_time=end_date_time, stage=item.stage, conductance=item.conductance
                    )
            return None
