...
def calculate_hob_package_data(modflow_model: ModflowModel) -> HobPackageData | None:
    if len(modflow_model.observations) == 0:
        return None

    head_observation_data = calculate_observation_items(modflow_model)
...
def create_hob_package(flopy_modflow: FlopyModflow, modflow_model: ModflowModel) -> FlopyModflowHob | None:
    package_data = calculate_hob_package_data(modflow_model)
