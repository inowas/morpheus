...
from morpheus.modflow.types.boundaries.Boundary import BoundaryType, RiverBoundary
from morpheus.modflow.types.boundaries.RiverObservation import RiverDataItem
                ...
                for layer_idx in layer_indices:
                    sp_data.set_value(time_step=stress_period_idx, layer=layer_idx, row=cell.y, column=cell.x,
                                      values=[river_stage.to_float(), conductance.to_float(),
                                              riverbed_bottom.to_float()], sum_to_existing=False)
            ...
            for cell in riv_boundary.affected_cells:
                ...
                for layer_idx in layer_indices:
                    sp_data.set_value(time_step=stress_period_idx, layer=layer_idx, row=cell.y, column=cell.x,
                                      ...
                                      sum_to_existing=False)

    return sp_data


def calculate_stress_period_data(model: ModflowModel) -> RivStressPeriodData | None:
    sp_data = RivStressPeriodData()
