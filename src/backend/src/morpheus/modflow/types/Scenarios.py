...
from .ModflowModel import ModflowModel, ModelId
...
class Scenario:
    ...
    def apply_base_model(self, base_model: ModflowModel) -> 'ModflowModel':

        # merge boundaries
        merged_boundaries = self.boundaries
        for scenario_boundary in self.boundaries:
            merged_boundaries = base_model.boundaries.with_added_or_updated_boundary(scenario_boundary)

        for removed_boundary_id in self.removed_boundaries:
            merged_boundaries = base_model.boundaries.with_removed_boundary(removed_boundary_id)
        ...
        return ModflowModel(
            ...
            observations=base_model.observations,
            soil_model=base_model.soil_model,
            transport=base_model.transport,
            variable_density=base_model.variable_density
        )
