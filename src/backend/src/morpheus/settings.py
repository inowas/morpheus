...
class Settings:
    def __init__(self, env: str, values):
        ...
        self.MONGO_PROJECT_DATABASE: str = values.MONGO_PROJECT_DATABASE
        ...
        self.OPENAPI_BUNDLED_SPEC_FILE: str = os.path.abspath(os.path.join(os.path.dirname(__file__), 'openapi.bundle.yml'))
...
settings = Settings.from_dynaconf(
    Dynaconf(
        ...
        validators=[
            ...
            Validator('MONGO_PROJECT_DATABASE', must_exist=True),
            Validator('MONGO_SENSOR_DATABASE', must_exist=True),
            Validator('MORPHEUS_PROJECT_ASSET_DATA', must_exist=True),
            Validator('MORPHEUS_PROJECT_CALCULATION_DATA', must_exist=True),
            Validator('MORPHEUS_SENSOR_LOCAL_DATA', must_exist=True),
