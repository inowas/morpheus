...
@router.get('', response_model=SensorListResponse, operation_id='read_sensors')
def read_sensors():
    return ReadSensorListRequestHandler.handle()[0]


@router.get('/latest', response_model=SensorsLatestValuesResponse, operation_id='read_sensors_latest_values')
def read_sensors_latest_values():
    return ReadSensorsLatestValuesRequestHandler.handle()[0]


@router.get('/project/{project}/sensor/{sensor}/parameter/{parameter}', response_model=SensorDataResponse, operation_id='read_sensor_data')
