...
function prepareBackendEnvFile {
  # load env variables from local infrastructure environment
  ...
  cp "$localInfrastructureRoot/.env" "$backendEnvFile.tmp" \
    ...
    && echo "" >> "$backendEnvFile" \
    && echo "BACKEND_MORPHEUS_SENSOR_LOCAL_DATA=$LOCAL_SENSOR_DATA_MOUNTPOINT" >> "$backendEnvFile" \
    && echo "BACKEND_MORPHEUS_PROJECT_ASSET_DATA=$LOCAL_PROJECT_ASSET_DATA_MOUNTPOINT" >> "$backendEnvFile" \
    && echo "BACKEND_MORPHEUS_PROJECT_CALCULATION_DATA=$LOCAL_PROJECT_CALCULATION_DATA_MOUNTPOINT" >> "$backendEnvFile" \
